...
我们可以从上述几个方面入手，来了解 Gdscript 脚本的语法约定。
...
面向对象，不是简单的执行结果，而是一种编程思想`它将程序设计视为对现实世界理解和抽象，并利用类和对象来描述和实现程序。` 其基础概念如下：

- **类 Class**：是指具有相同特征`对象的属性和方法`的一群对象。
- **实例对象 Object**：是指通过类创建的对象。
- **封装**：封装是指将类的属性和方法封装在一起，只提供必要的访问方式。
- **继承**：继承是指一个类继承另一个类的属性和方法。
- **多态**：多态是指一个类具有多个不同实现的子类。
